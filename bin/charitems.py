def to_binary(string):
    """
    Converts an itemset string to a binary representation
    'a'   -> 1 (0001)
    'b'   -> 2 (0010)
    'aba' -> 3 (0011)
    ... 
    ...
    int_rep = 0
    for s in string:
        # eg 'c' should have position 3 from the left in a bit string: 0100
...
def to_chars(binary):
    """ Converts binary representations of an itemset to string """
    string = ""
    for t in range(10):
        bit = 2 ** t
        if binary & bit == bit:
